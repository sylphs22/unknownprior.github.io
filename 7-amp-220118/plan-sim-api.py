...
@app.route('/const_sim', methods=['POST'])
def main():
    ...
    if (app.target_output == 'Itunes Downloads'):
       ...
       app.csv=pd.read_csv('CSV/artist_model_itunes_simdb.csv')
       ...
       app.csv=pd.read_csv('CSV/artist_model_spotify_simdb.csv')
...
def sim(budget) :
    ...
    res_out = []
    np.random.seed(0)
    sample = np.random.dirichlet(np.ones(11),size=30) 
    # import pdb; pdb.set_trace()
    #results = pool.map(sim_mapper, sample)
    with ThreadPoolExecutor (max_workers = 30) as executor:
        futures = [executor.submit(sim_mapper,s, budget) for s in sample]
    for f in futures:
        try : 
            res = f.result()
...
def sim_mapper(sample,budget):
	...
	ub = np.array (ub)
	ab = (lb+ub)/2 
	return minimize(f, ub,  method='slsqp', constraints = cons,bounds = bounds,jac = False)
#    return res
...
def f(x):
    ...
    col2 = ['paid_youtube_impressions',
       ...
       'jet_capital_impacts', 'jet_kiss_impacts', 'jet_heart_impacts','trend']
